...
# Old hand-written pages in blog/ that a newer post replaces.
#   key   = the stale file's slug in blog/ (it has no posts/*.md source, so this
#           build can neither regenerate nor remove it on its own)
#   value = the slug of the post that supersedes it
#
# Nothing happens while the successor is still scheduled — the old page stays
# exactly as it is. On the first build after the successor publishes, the stale
# file is deleted and a 301 appears in the generated block of _redirects. That
# timing is the whole point: adding the redirect by hand today would send
# readers to a 404 until the successor's date arrives, and adding it by hand on
# the day means remembering to.
#
# ozempic-muscle-loss -> glp1-muscle-loss (publishes 2026-08-12): same subject,
# and the older piece carries no inline citations at all — one bare source under
# a "Sources & further reading" heading — against 12 markers in the new one. On
# a site whose stated promise is that cited claims have been checked, the
# uncited version is not the one to keep.
SUPERSEDED = {"ozempic-muscle-loss": "glp1-muscle-loss"}

REDIRECTS_FILE = ROOT / "_redirects"
GENERATED_BEGIN = "# BEGIN generated by build.py — edits below this line are overwritten"
GENERATED_END = "# END generated by build.py"


def retire_superseded(posts_by_slug):
    """Delete stale blog pages whose replacement is live, and 301 them to it.

    Only the block between the generated markers is rewritten; everything a
    human wrote above it is preserved byte for byte.
    """
    # A successor slug that matches no post source will never fire, and the
    # symptom is silence — the old page just stays up forever. That is exactly
    # how build.py went on waiting for "fibre-and-your-own-glp1" after the file
    # was renamed to "fiber-". Say so loudly instead.
    sources = {p.stem for p in POSTS_DIR.glob("*.md")}
    for old, new in sorted(SUPERSEDED.items()):
        if new not in sources:
            print(f"  WARNING  SUPERSEDED['{old}'] names '{new}', which has no "
                  f"posts/{new}.md — this redirect can never fire")

    live = {old: new for old, new in SUPERSEDED.items()
            if new in posts_by_slug and (OUT_DIR / f"{new}.html").exists()}

    for old, new in sorted(live.items()):
        stale = OUT_DIR / f"{old}.html"
        if stale.exists():
            stale.unlink()
            print(f"  retired blog/{old}.html -> /blog/{new}  (301)")

    text = REDIRECTS_FILE.read_text(encoding="utf-8")
    handwritten = text.split(GENERATED_BEGIN)[0].rstrip("\n")

    if not live:
        # Nothing to emit. Drop any stale generated block rather than leaving a
        # redirect for a page that is no longer superseded.
        REDIRECTS_FILE.write_text(handwritten + "\n", encoding="utf-8")
        return

    lines = [handwritten, "", GENERATED_BEGIN,
             "#", "# Retired pages, from SUPERSEDED in build.py. Each source page has been",
             "# deleted from blog/; these keep its URL working and pass its history to the",
             "# article that replaced it. To undo one, remove its entry from SUPERSEDED.", "#"]
    for old, new in sorted(live.items()):
        lines.append(f"/blog/{old}.html  /blog/{new}  301")
        lines.append(f"/blog/{old}       /blog/{new}  301")
...
def main():
    ...
    retire_superseded(posts_by_slug)
